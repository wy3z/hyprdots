...
-- workspace-switch: slide axis follows the focused monitor (vertical on landscape,
-- horizontal on portrait). Hyprland animations are global, so flip the 'workspaces'
-- style on every monitor.focused event. The _G guard drops the previous
-- subscription so `hyprctl reload` doesn't stack hooks.
...
local function apply_ws_anim_axis(m)
    -- monitor.focused passes the newly-focused monitor; use it directly (the active
    -- monitor is still the one being LEFT at event time). Timer path passes nil.
    m = m or hl.get_active_monitor()
    if not m then return end
    local ew, eh = m.width, m.height
    if m.transform % 2 == 1 then ew, eh = m.height, m.width end -- 90/270-rotated: swap w/h
    ws_anim.style = (eh > ew) and "slide" or "slidevert"        -- portrait=horizontal, landscape=vertical
    hl.animation(ws_anim)
end
if _G.__ws_anim_sub then _G.__ws_anim_sub:remove() end
_G.__ws_anim_sub = hl.on("monitor.focused", apply_ws_anim_axis)
-- monitors aren't ready at config eval (active monitor is nil), so defer the initial apply.
